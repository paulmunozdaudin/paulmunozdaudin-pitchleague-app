...
from app.schemas.ai import MatchInsightOut, ModelProbabilities
...
def get_match_insight(
    ...
    text, text_provider, prediction = generate_match_insight(db, match)
    return MatchInsightOut(
        match_id=match.id,
        summary=text,
        provider=text_provider,
        model=ModelProbabilities(
            home=prediction.home_prob, draw=prediction.draw_prob, away=prediction.away_prob, source=prediction.source
        ),
    )
